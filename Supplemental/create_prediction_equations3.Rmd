...
data = df %>% select(days.ath.atl, pct.ooath.ooatl , pct.ooatl.oath,  pct.oath.oatl , pct.oatl.ath  
,pct.ooath.oath  ,    pct.oath.ath  ,  pct.ooatl.oatl  ,  days.ooath.ooatl   
, days.ooatl.oath , days.oath.oatl  , days.oatl.ath  ,  days.ooath.oath    
,days.oath.ath , days.ooatl.oatl  ,reg.ooath.ooatl ,reg.ooatl.oath , reg.oath.oatl      
,reg.oatl.ath  ,reg.ooath.oath ,reg.oath.ath, reg.ooatl.oatl     
,bad.months  ,reg.ath.atl,ath.bull ,order) %>% na.omit()


# data2 = df %>% select(days.ath.atl, log.pct.ooath.ooatl , log.pct.ooatl.oath ,log.pct.oath.oatl  , log.pct.oatl.ath ,log.pct.ooath.oath , log.pct.oath.ath , log.pct.ooatl.oatl, log.days.ooath.ooatl   
# ,log.days.ooatl.oath, log.days.oath.oatl  ,  log.days.oatl.ath  , log.days.ooath.oath    
# ,log.days.oath.ath  , log.days.ooatl.oatl  ,log.reg.ooath.ooatl , log.reg.ooatl.oath ,log.reg.oath.oatl      ,log.reg.oatl.ath ,log.reg.ooath.oath , log.reg.oath.ath  ,log.reg.ooatl.oatl     
# ,bad.months    ,log.reg.ath.atl , ath.bull, order) %>% na.omit()


algorithms = data.frame(model = c("CART",
	"MARS","Bagged CART","Bagged MARS","Random Forest","Random Forest 2","Boosted Tree","Extreme Gradient Boost 1"),
	code =  c("rpart","earth","treebag","bagEarth","rf","ranger","bstTree","xgbLinear"))

model=list()
for(i in 1:nrow(algorithms)){
model[[i]] <- train(days.ath.atl~., data=data, method=algorithms$code[i])
}
names(model) = algorithms$model

 results=resamples(model)$values %>% select(contains("MAE"))
 colnames(results) = algorithms$model
 mae = results %>% gather(model,MAE) %>% group_by(model) %>%
  	summarize(MAE_mean = mean(MAE), MAE_sd = sd(MAE)) %>% arrange(MAE_mean) 

 
 d.fit.ml = model
save(d.fit.ml,file="models/d.fit.ml.Rda")
